...
**기프티허브**는 바코드 기반의 디지털 기프티(상품권) 플랫폼입니다.

사용자는 앱에서 브랜드별 기프티콘을 구매하고, 바코드를 제시하여 가맹점에서 사용합니다. 사용하지 않은 기프티는 다른 사용자에게 양도하거나, 중고마켓에서 직접 가격을 정해 판매할 수 있습니다. 가맹점은 같은 앱을 설치하되, 가맹점 계정으로 가입하면 QR 스캔 및 정산 전용 화면이 제공됩니다.
...
- **상품 가격**: KRW 기반 (브랜드별 기프티콘, 할인율 표시) / 결제는 PayPal·SmileyPay·USDT 사용
- **바코드**: 암호화된 바이너리 기반, 사용 시마다 자동 갱신되어 보안 유지 (30초 주기 갱신)

### 0.2 사용자(User) 화면 흐름

> 하단 탭: `[ 스토어 ] [ 중고마켓 ] [ 내 기프티 ] [ 장바구니 ] [ MY ]`
...
| 단계 | 화면 | WF | 설명 |
|------|------|-----|------|
| 메인 | 홈 | WF 2.1 | 보유 기프티 잔액 합계, 기프티 목록(최대 3개), 스토어/중고마켓 바로가기 |

#### 스토어 탭 (첫 번째 탭)

| 단계 | 화면 | WF | 설명 |
|------|------|-----|------|
| 목록 | 스토어 | WF 2.2 | 예산별 패키지 생성 섹션 + 브랜드별 기프티콘 그리드 (KRW, 할인율 표시) |
| 상세 | 상품 상세 | WF 2.2.1 | 상품 정보, 수량 선택, 선물하기/장바구니/구매하기 |
| 패키지 | 예산별 패키지 생성 | WF 2.2.2 | 금액 입력 → 상품 조합 추천 → 수량 조절 → 선물하기/장바구니/구매하기 |
| 구매 | 결제 진행 | WF 2.3 | 결제 수단 선택(PayPal/SmileyPay/USDT), 결제 확인 대기 |
| 선물 | 선물하기 | WF 2.4 | 보내는 사람(텔레그램 ID), 받는 사람(텔레그램 ID 검색), 결제 수단 선택, 수수료 1% |

#### 기프티 관리

| 단계 | 화면 | WF | 설명 |
|------|------|-----|------|
| 상세 | 기프티 상세 | WF 2.5 | 바코드 표시(복사 가능), 만료일, 양도 횟수, 상태. 저장하기/양도/환불/판매등록/바코드 재발급 진입점 |
...
| 단계 | 화면 | WF | 설명 |
|------|------|-----|------|
| 목록 | 내 기프티 | WF 2.8 | 필터 탭: 전체 / 판매중 / 사용가능 / 사용중 / 기간만료 / 받은선물 / 보낸선물 |
| 상세 | 기프티 상세 | WF 2.5 | (위 참조) |

#### 중고마켓 탭

| 단계 | 화면 | WF | 설명 |
|------|------|-----|------|
| 목록 | 중고마켓 목록 | WF 2.10 | 검색, 카테고리 탭(전체/방문권/상품권/금지/뷰티), 판매 중인 기프티 그리드 |
| 상세 | 중고 상품 상세 | WF 2.11 | 브랜드, 상품명, 원가(KRW), 할인율, 판매가(KRW), 구매 버튼 |
| 구매 | 중고 구매 결제 | WF 2.14 | 결제 수단 선택(PayPal/SmileyPay/USDT), 결제 확인 대기 |
| 완료 | 중고 구매 완료 | WF 2.15 | 구매 완료 안내, 내 기프티에 추가됨 확인 |
| 판매 | 판매 등록 | WF 2.12 | 보유 상품 목록에서 상품 선택 + 수량 설정, 수수료(5%) 미리보기, 정산 예정금액 확인 |
...
| 단계 | 화면 | WF | 설명 |
|------|------|-----|------|
| 목록 | 장바구니 | WF 2.9 | 단일 상품 + 패키지 상품 목록, 수량 조절, 총 금액, 선물하기/구매하기 선택 |

#### MY 탭

| 단계 | 화면 | WF | 설명 |
|------|------|-----|------|
| 설정 | 마이페이지 | WF 2.9.1 | 프로필, 계정 설정, 알림 설정, 보안 설정, 고객 지원, 로그아웃 |
...
| 바코드 재발급 확인 | WF 4.3 | 기존 바코드 무효화 + 새 바코드 발급 안내 |
...
| 단계 | 화면 | WF | 설명 |
|------|------|-----|------|
| 스캔 | QR 스캔 (홈) | WF 3.1 | 카메라로 고객 QR 스캔, 오늘의 매출/평균 매출/처리 건수 요약 |
...
```
[ 스토어 ] [ 중고마켓 ] [ 내 기프티 ] [ 장바구니 ] [ MY ]
```
> 스토어가 첫 진입 탭. 홈 화면은 스토어 탭 내에서 별도 접근 가능.
> 사용내역 탭 제거, 장바구니 탭 신규 추가 (배지로 담긴 상품 수 표시).

**가맹점 계정 (account_type: "merchant")**
```
[ QR스캔 ] [ 내역 ] [ 정산 ] [ 설정 ]
```
> 사용처리 탭 제거 — 사용 처리는 QR스캔 탭에서 스캔 후 자동 연결됨.
...
- 결제 완료 시 바코드 포함 기프티 발급
- 구매 내역 기록 (자동 로그)
- **브랜드별 기프티콘**: KRW 가격 기반, 할인율 표시
- **예산별 패키지 추천**: 금액 입력 → 조합 추천 → 구매
...
#### 5.2.2 예산별 패키지 추천
사용자가 예산 금액을 입력하면 최적 조합을 자동 추천한다.

| 단계 | 설명 |
|------|------|
| 1. 금액 입력 | 예산 입력 후 "패키지 추천" 버튼 |
| 2. 조합 추천 | 고가 상품부터 채운 뒤 나머지 예산으로 소액 상품 추가 |
| 3. 수량 조절 | 추천 결과에서 상품별 수량 조절 가능 |
| 4. 선택 | 선물하기 / 장바구니 / 구매하기 중 선택 |

#### 5.2.3 선물하기
구매 시 바로 다른 사용자에게 기프티콘을 선물할 수 있다.

| 항목 | 설명 |
|------|------|
| 수신자 검색 | 텔레그램 ID로 검색 |
| 수수료 | 결제 금액의 1% |
| 결제 수단 | PayPal / SmileyPay / USDT |
| 진입 경로 | 상품 상세 → 선물하기 / 패키지 추천 → 선물하기 / 장바구니 → 선물하기 |

### 5.3 기프티 사용
- **전액 사용**: 기프티 금액 전체 사용, 상태를 '사용완료'로 변경
- **일부 금액 사용**: 사용 금액만큼 차감, 잔액 유지
- **기프티 상세**: 바코드 표시(번호 복사 가능), 저장하기 기능, 양도/환불/판매등록/바코드 재발급 진입점
- **사용 흐름**:
  1. 사용자가 앱에서 바코드 제시
  2. 가맹점이 가맹점 앱으로 QR 스캔
...
[오프라인 시 동작]
  ...
  ❌ 불가능한 것 (서버 통신 필수):
  ├── 기프티 구매 (결제)
  ├── 기프티 사용 처리 (바코드 스캔 → 서버 TOTP+qr_token 검증)
  ├── 양도 / 환불 요청
  ├── 바코드 수동 갱신
  └── 가맹점: 바코드 스캔 및 사용 처리
```

> **사용(결제) 처리는 반드시 온라인 상태에서만 가능** — TOTP 검증 및 qr_token 대조가 서버에서만 이루어지므로 오프라인 사용은 원천적으로 불가
...
| 기프티 상세 + 바코드 이미지 | 상세 화면 진입 시 | AsyncStorage / MMKV |
...
- **카테고리 탭**: 전체 / 방문권 / 상품권 / 금지 / 뷰티

#### 5.12.2 판매 등록 흐름
1. 판매자: 중고마켓 탭에서 "판매하기" 버튼 탭 (우측 하단 FAB)
2. 보유 기프티 목록에서 판매할 상품 선택 (복수 선택 가능)
3. 상품별 수량 설정 (다이얼로그로 최대 보유 수량까지 설정)
4. 판매가(KRW) 자동 설정 (원가의 약 90%), 수동 조절 가능
5. 총 판매금액 / 수수료(5%) / 정산 예정금액 미리보기 확인
6. "상품등록하기" → 기프티 status: `on_sale` (사용/양도/환불 잠금)
7. 중고마켓 목록에 노출
8. 판매 취소 가능 → status: `active` 복원

#### 5.12.3 구매 흐름
1. 구매자: 중고마켓에서 기프티 선택 (그리드 형태, 브랜드/상품명/원가/할인율/판매가 표시)
2. 결제 금액(KRW) 확인 + 수수료 표시
...
## 6. 바코드 설계 (1D 바코드 + TOTP)

### 6.1 설계 원칙

- **사용자 앱 표시**: 14자리 숫자 기반 1D 바코드 (Code 128), 30초마다 자동 갱신
- **가맹점 스캔**: 카메라로 바코드 스캔 → 서버 2단계 검증
- **2단계 검증 구조**:
  - 1단계: TOTP 검증 (account_id + device_id + voucher_id 기반) — 위조 바코드 차단
  - 2단계: qr_token으로 voucher 상태 검증 — 잔액/만료/활성 확인
- **voucher_id 노출 방지**: FPE(Format-Preserving Encryption)로 변형하여 바코드에 표시
- 서버 마스터 키만 보유, 앱에는 시크릿 없음
...
```
┌──────────────────────────┬──────────────┐
│  FPE(voucher_id) (8자리) │ TOTP (6자리) │
│       ex) 73829461       │  ex) 582947  │
└──────────────────────────┴──────────────┘
              = 14자리 숫자 (Code 128 바코드)
```

- `FPE(voucher_id)`: DB PK를 Format-Preserving Encryption으로 변형한 8자리 숫자
  - 순번 노출 없음 (12345 → 73829461, 비가역적으로 보임)
  - 서버만 역변환 가능, 길이·자릿수 유지
- `TOTP`: 30초 윈도우 기반 6자리 OTP (매 30초마다 자동 변경)
- 14자리 숫자 → Code 128 Numeric → 1D 바코드로 충분히 스캔 가능

**FPE 방식 (FF3-1, NIST SP 800-38G)**

```
encoded = FF3_1_encrypt(BARCODE_KEY, voucher_id, radix=10, len=8)

- BARCODE_KEY : Workers 환경변수, 서버만 보유
- 입력/출력 모두 10진수, 길이 고정 (8자리)
- 역변환: voucher_id = FF3_1_decrypt(BARCODE_KEY, encoded)
- Web Crypto API 미지원 → JS 구현 (AES 기반 8라운드, ~0.1–0.5ms)
```
...
voucher마다 고유한 시크릿을 가져 하나가 탈취되어도 다른 voucher에 영향 없음.

```
[앱 / 서버 양쪽에서 동일하게 계산]

totp_secret = HMAC-SHA256(MASTER_KEY,
                           account_id + device_id + voucher_id)

- MASTER_KEY  : Workers 환경변수, 서버만 보유
- account_id  : 사용자 고유 ID (accounts.account)
- device_id   : 기기 고유 ID (accounts.device_id)
- voucher_id  : 실제 DB PK (FPE 역변환 후 값)

→ 결정론적 파생 — 서버가 별도 저장 없이 언제든 재계산 가능
→ 세 값 중 하나라도 달라지면 시크릿도 완전히 달라짐
```

**TOTP 값 생성 (RFC 6238)**

```
T      = floor(Unix timestamp / 30)      ← 30초 윈도우
TOTP   = HOTP(totp_secret, T) % 10^6    ← 6자리 숫자
허용범위: T-1, T, T+1  (±30초 시계 오차 허용)
```

---

### 6.4 서버 내부 qr_token (2단계 검증용)

TOTP 통과 후 voucher의 실제 유효성을 검증하는 내부 토큰.

```
[이벤트 발생 시 서버에서 생성]

seed     = SHA-256(device_id + account_id + timestamp)
qr_token = HMAC-SHA256(QR_ENCRYPTION_KEY, seed)  ← 32 bytes, DB 저장
```

| 시점 | 동작 |
|------|------|
| 기프티 구매 완료 | 최초 qr_token 생성, DB 저장 |
| 사용자가 바코드 조회 | 기존 qr_token 그대로 반환 (변경 없음) |
| 양도 완료 | 기존 qr_token 무효화, 수신자 기준 새 토큰 생성 |
| 사용 처리 완료 | 사용된 토큰 무효화, 잔액 남으면 새 토큰 생성 |
| 바코드 수동 갱신 | 기존 무효화 + 새 토큰 생성 (분실/유출 대응) |
| 중고거래 완료 | 기존 무효화, 구매자 기준 새 토큰 생성 |
...
바코드 표시                               카메라로 스캔
73829461 582947          ─────────→     14자리 숫자 추출
(30초마다 TOTP 갱신)                          │
                                        POST /api/merchant/scan
                                        { barcode: "73829461582947" }
                                              │
                                    ┌─────────┴──────────┐
                                    │        서버          │
                                    │                    │
...
| 연산 | 방식 | 예상 시간 |
|------|------|---------|
| FPE 역변환 (FF3-1) | JS 구현, AES 8라운드 | ~0.1–0.5ms |
| D1 단일 쿼리 | indexed SELECT, 모든 필드 1회 조회 | ~1–3ms |
| HMAC-SHA256 (TOTP 시크릿) | Web Crypto API (하드웨어 가속) | ~0.01ms |
| TOTP 계산 + 3윈도우 대조 | HOTP × 3 + 비교 | ~0.03ms |
| qr_token 대조 | 메모리 비교 (DB 결과 재사용) | ~0ms |
| **합계** | | **~2–5ms** |

> D1 I/O가 전체 시간의 대부분을 차지. 암호 연산은 무시할 수준.
> D1 조회를 2회 → 1회로 줄여 불필요한 왕복 제거.

#### 상황별 예상 지연

| 상황 | 예상 시간 | 원인 |
|------|---------|------|
| 단일 스캔 (일반) | 2–5ms | D1 I/O 지배 |
| 단일 스캔 (D1 cold) | 15–25ms | 첫 연결 초기화 |
| 동시 스캔 다수 | 각 2–7ms (병렬) | Workers 격리 실행, 암호 연산 병렬 |
| D1 쓰기 경합 피크 | 50–100ms | 동시 use 처리 시 write lock |

> Workers는 요청마다 격리 실행되므로 암호 연산은 동시 요청 수와 무관하게 독립적.
> 100ms 지연은 D1 쓰기 경합 발생 시의 최악 케이스.
...
| 항목 | 설명 |
|------|------|
| 바코드 포맷 | Code 128, 14자리 숫자 (FPE(voucher_id) 8 + TOTP 6) |
| FPE 알고리즘 | FF3-1 (NIST SP 800-38G), AES 기반, radix=10 |
| TOTP 알고리즘 | RFC 6238 (HOTP 기반, SHA-256, 30초 윈도우) |
| TOTP 시크릿 | HMAC-SHA256(MASTER_KEY, account_id + device_id + voucher_id) |
| voucher별 시크릿 분리 | voucher_id 포함으로 voucher마다 다른 시크릿 |
| 시간 오차 허용 | ±1 윈도우(±30초) — T-1, T, T+1 검증 |
| qr_token | HMAC-SHA256(QR_ENCRYPTION_KEY, seed) — 32 bytes, 이벤트 기반 갱신 |
| 기기 바인딩 | device_id 포함 → 다른 기기에서 생성 불가 |
| 사용자 바인딩 | account_id 포함 → 계정만 탈취해도 기기 없으면 불가 |
| 스캔 Rate Limit | 가맹점 계정당 1초 1회, 실패 5회 시 1분 잠금 |
| 키 로테이션 | MASTER_KEY / BARCODE_KEY / QR_ENCRYPTION_KEY 주기적 교체, 이전 키 병행 유지 |
...
기기 변경 (앱 재설치 / 기기 교체)
        ...
        ├── device_id 변경
        │       │
        │       ├── TOTP 시크릿 변경 → 기존 바코드 즉시 무효
        │       └── 본인 인증(이메일) 후 서버에 새 device_id 등록
        │
        └── 새 device_id 기준으로 qr_token 재생성 + 바코드 재발급
```

- 이전 기기의 바코드는 TOTP 불일치로 자동 차단
- FPE encoded 값은 변하지 않음 (voucher_id 불변) — TOTP만 무효화
...
공격자가 바코드 스크린샷 획득
        │
        ├── FPE(voucher_id): 알 수 있으나 실제 voucher_id 역산 불가 (키 없음)
        └── TOTP: 30초 내 유효, 이후 자동 만료
                │
                ├── 30초 내 사용 시도 → 가맹점 앱 인증 필요, 현실적으로 불가
                └── 30초 경과 → TOTP 불일치, 자동 차단
```

캡처만으로는 사용 불가. 기기 탈취 + 앱 접근 + TOTP 윈도우 동시 충족이 필요한 구조.
